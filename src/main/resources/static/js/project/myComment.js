$(document).ready(function() {
    $(".myText-div").click(function() {
        let clickedValue = $(this).find('.myText-content-title').text();
        let target = $(this).data("bs-target");

        $(target).on('shown.bs.offcanvas', function() {
            $('.comment-li').each(function() {
                let commentText = $(this).find('.comment-text').text();
                if (commentText === clickedValue) {
    ...
    $('.myText-div').click(function (e) {
        ...
        if ($(this).attr('id') === 'board-text'){
            ...
            $.ajax({
                ...
                data: formData,
                success: (result) => {
                    showBoardText(result, clickedValue)
                }
            })
            ...
            $.ajax({
                ...
                data: formData,
                success: (result) => {
                    showTaskText(result, clickedValue)
                }
            })
            ...
            $.ajax({
                ...
                data: formData,
                success: (result) => {
                    showCalendarText(result, clickedValue)
                }
            })
        }
...
function showTaskText(result, clickComment) {
    ...
    $.ajax({
        ...
        success: (comment) => {
                ...
                for (let i = 0; i < comment.length; i++) {
                    ...
                    let entryDate = new Date(date.getTime() + TIME_ZONE).toISOString().replace('T', ' ').slice(0, -5);
                        ...
                        $('#commentGroup-myText').append("<li class='comment-li'><div class=\"comment-thumbnail\">\n" +
                            ...
                            "                                            <button type=\"button\" class=\"modify-tComment comment-writer-button on\" style=\"background-color: transparent; border: none\" onclick=\"modifyTextTComment(this)\">\n" +
                            "                                                수정</button>\n" +
                            "                                            <button type=\"button\" class=\"delete-tComment comment-writer-button on\" style=\"background-color: transparent; border: none\" onclick=\"deleteTextTComment(" + comment[i].tCommentId + ")\">\n" +
                            "                                                삭제</button>\n" +
                            "                                        </div>\n" +
                            "                                    </div>\n" +
                            "                                    <div class=\"comment-content\">\n" +
                            "                                        <div class=\"comment-text-area\">\n" +
                            "                                            <div class=\"js-remark-text comment-text-myComment\">" + comment[i].tComment + "</div>\n" +
                            "                                             <select id='translation-myComment' class=\"form-select form-select-sm translation-myComment\" aria-label=\".form-select-sm example\" >\n" +
                            ...
                            "                                    <form  action=\"/tComment/updateText\" method=\"post\" class=\"comment-container\" style=\"padding: 0;\">\n" +
            ...
            $('.comment-li').each(function() {
                let commentText = $(this).find('.comment-text').text()
                if (commentText === clickComment) {
                    $(this).css('border', '3px solid red');
                }
            });
            $('.show-comment-myComment').each(function () {
                let commentContainer = $(this).find('.comment-container');
                let selectElement = commentContainer.find('.translation-myComment');
                selectElement.on('change', function () {
                    let selectedValue = $(this).val();
                    console.log(selectedValue);
                    let commentText = $(this).siblings('.comment-text-myComment');
                ...
                for (let i = 0; i < comment.length; i++) {
                    ...
                    let entryDate = new Date(date.getTime() + TIME_ZONE).toISOString().replace('T', ' ').slice(0, -5);
                        $('#commentGroup-myText-board').append("<li class='comment-li'><div class=\"comment-thumbnail\">\n" +
                            ...
                            "                                        </div>\n" +
                            "                                        <div class=\"comment-writer-menu\">\n" +
                            "                                            <button type=\"button\" class=\"modify-tComment comment-writer-button on\" style=\"background-color: transparent; border: none\" onclick=\"modifyTextBComment(this)\">\n" +
                            "                                                수정</button>\n" +
                            "                                            <button type=\"button\" class=\"delete-tComment comment-writer-button on\" style=\"background-color: transparent; border: none\" onclick=\"deleteTextBComment(" + comment[i].bCommentId + ")\">\n" +
                            "                                                삭제</button>\n" +
                            "                                        </div>\n" +
                            "                                    </div>\n" +
                            "                                    <div class=\"comment-content\">\n" +
                            "                                        <div class=\"comment-text-area\">\n" +
                            "                                            <div class=\"js-remark-text comment-text-myComment\">" + comment[i].bComment + "</div>\n" +
                            "                                             <select id='translation-myComment' class=\"form-select form-select-sm translation-myComment\" aria-label=\".form-select-sm example\" >\n" +
                            "                                                   <option value='' disabled selected>번역하기</option>\n" +
                            "                                                   <option value=\"en\">English</option>\n" +
                            "                                                   <option value=\"ko\">korean</option>\n" +
                            ...
                            "                                    <form  action=\"/bComment/updateText\" method=\"post\" class=\"comment-container\" style=\"padding: 0;\">\n" +
...
function deleteTextBComment(commentId) {
    Swal.fire({
        text: '댓글을 삭제하시겠습니까?',
        width: '300px',
...
function deleteTextTComment(commentId) {
    Swal.fire({
        text: '댓글을 삭제하시겠습니까?',
        width: '300px',
        showCancelButton: true,
...
function showCalendarText(result, clickComment) {
    ...
    //작성일
    $('.myText-postDate').text(date)
    if (result.calAddress === '' || result.calAddress == null) {
        ...
        success: (comment) => {
            ...
            for (let i = 0; i < comment.length; i++) {
                ...
                $('#commentGroup-myText-calendar').append("<li class='comment-li'><div class=\"comment-thumbnail\">\n" +
                    ...
                    "                                        </div>\n" +
                    "                                        <div class=\"comment-writer-menu\">\n" +
                    "                                            <button type=\"button\" class=\"modify-cComment comment-writer-button on\" style=\"background-color: transparent; border: none\" onclick=\"modifyTextCComment(this)\">\n" +
                    "                                                수정</button>\n" +
                    "                                            <button type=\"button\" class=\"delete-cComment comment-writer-button on\" style=\"background-color: transparent; border: none\" onclick=\"deleteTextCComment(" + comment[i].calCommentId + ")\">\n" +
                    "                                                삭제</button>\n" +
                    "                                        </div>\n" +
                    "                                    </div>\n" +
                    "                                    <div class=\"comment-content\">\n" +
                    "                                        <div class=\"comment-text-area\">\n" +
                    "                                            <div class=\"js-remark-text comment-text-myComment\">" + comment[i].calComContent + "</div>\n" +
                    "                                             <select id='translation-myComment' class=\"form-select form-select-sm translation-myComment\" aria-label=\".form-select-sm example\" >\n" +
                    "                                                   <option value='' disabled selected>번역하기</option>\n" +
                    "                                                   <option value=\"en\">English</option>\n" +
                    "                                                   <option value=\"ko\">korean</option>\n" +
                    ...
                    "                                    <form  action=\"/cComment/updateText\" method=\"post\" class=\"comment-container\" style=\"padding: 0;\">\n" +
...
function deleteTextCComment(commentId) {
    Swal.fire({
        text: '댓글을 삭제하시겠습니까?',
        width: '300px',
        showCancelButton: true,
